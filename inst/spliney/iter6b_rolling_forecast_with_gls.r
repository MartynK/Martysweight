...
out <- expand.grid(rmse = NA,
                  mad = NA,
                  cor. = NA,
                  k = seq(2,10,length.out=3) %>% round,
                  days_before_last_perc = seq(.01, 1, length.out = 3)
                  ) %>%
...
out_groupped %>%
  ggplot(aes(x = days_before_last_perc, 
             y = rmse_mean, 
             group = days_before_last,
             color= k)) +
  theme_bw() +
  geom_point() +
  geom_line(mapping = aes(group=k),alpha = .5) 
