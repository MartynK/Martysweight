...
- `anthropometrics_tidy.xlsx` -- Body-composition data ported from the personal filing workbook: one row per observation, one **sheet per person** (`Marci`, `Person_2`), English column names, plus `Notes` and `Reference` sheets. Derived columns (bmi, bf_jp3, bf_mod_jp3, bf_navy, bf_ymca, fat_mass, ffm, ffmi, ffmi_std, delta_*) are **live Excel formulas** with cached values baked in. Generated by `simple_summ_plot/port_anthropometrics.r`, read by `simple_summ_plot/iter2.r`. (Superseded `anthropometrics.xlsx`, a stale raw transposed dump, removed 2026-07-26.)
...
### inst/simple_summ_plot/ (clean summary outputs)
- `iter1.r` -- Machine-readable spline plot + quarterly body-mass summary (pred, deriv_kgd, change, high, low, range). Re-run by the `measure_weight` skill after every new weight entry.
- `iter2.r` -- Anthropometrics daily interpolation (monoH.FC spline blended with linear via `SPLINE_TENSION`, LOESS overlay for dense runs) + quarterly anthro summary
- `port_anthropometrics.r` -- One-off port of the filing workbook's Anthropometrics sheet into `extdata/anthropometrics_tidy.xlsx` (transposes, splits people onto separate sheets, re-creates the derived-metric equations as live Excel formulas). Falls back to a PowerShell copy because the OneDrive source is usually locked by Excel.

### inst/ (other studies)
- `fasttab.r` -- Quick fasting metric tabulation
- `2026.food_log_xplore/` -- Intra-day macro intake dynamics (burndown curves, ns model, Shiny explorer)
