...
$$T\ddot omeg = \beta_0 + \beta_1 \cdot Ballaszt + \beta_2 \cdot M\acute ert\:szitu\acute aci\acute o + \epsilon$$
...
$$M\acute ert\:T\ddot omeg = \beta_0 + \beta_1 \cdot Igazi\: t\ddot omeg + \epsilon$$
...
Az ébredéstől eltelt órákat is feljegyztem, ezek azonban nem tűntek relevánsnak. A megfigyeléseket első körben (*thin plate regresson*) GAM-modellel vizsgáltam, az ébren töltött órákat pedig a továbbiakban elhanyagolom.


$$
\begin{split}
M\acute ert\:t\ddot omeg=\beta_0+
\beta_1\cdot s(Koplal\acute assal\:t\ddot olt\ddot ott\:id\hat o)
+\beta_2\cdot s(Eltelt\:napok\:sz\acute ama)\\
+\beta_3\cdot s(\acute Etkez\acute es\:ut\acute an\:m\acute ert\:t\ddot omeg)
+\beta_4\cdot s(\acute Ebren\:t\ddot olt\ddot ott\:\acute or\acute ak\:sz\acute ama) + \epsilon
\end{split}
$$

ahol

$$s(x) = \sum_{i=1}^{K}\omega _i\varphi (\left \| x-c_i \right \|)$$
és

$$\varphi(r)=r^2log(r)$$

## Ébren töltött órák száma

Az ébren töltött órák száma nem tűnik fontos változónak, a továbbiakban elhanyagoljuk.  
...
Vegyük észre, hogy amíg a kalibrált görbénél 90 kg környékén a SE kb. *`r round( (marpred[3] - marpred[2])/(2*1.96), 2)`* kg, a GAM modell **\textcolor{blue}{prediktorainak}** a bizonytalansága sok esetben ennél kisebb. (Bár maga a modell nehezen vehető komolyan.)
...
Kevert modelleket vizsgáltam, ahol *\textcolor{blue}{korrellálatlan random slope/intercepttel}* jellemeztem a koplalás hatását. Egy-egy koplalási 'epizód' hatását random hatásként vizsgáltam (*i*).

$$
\begin{split}
M\acute ert\:t\ddot omeg_i =\beta_0 + \alpha_i + 
\sum_{k=1}^{d_1}\beta_{1k}\cdot B_k(Koplal\acute assal\:t\ddot olt\ddot ott\:id\hat o , q_d) \\
+ \sum_{k=1}^{d_1}\beta_{1k_i}\cdot B_k(Koplal\acute assal\:t\ddot olt\ddot ott\:id\hat o_i , q_d) \\
+\sum_{k=1}^{d_2}\beta_{2k}\cdot B_k(\acute Etkez\acute es\:ut\acute an\:m\acute ert\:t\ddot omeg, q_d) \\ 
+\sum_{k=1}^{d_3}\beta_{3k}\cdot B_k(Eltelt\:napok\:sz\acute ama, q_d)+\epsilon_i
\end{split}
$$
$$
\alpha_i \in \textsl{N}(0,\sigma_{episode}^2),\: \epsilon_{i} \in \textsl{N}(0,\sigma^2 {I_n}_{_i})
$$

$$
B_1(x)=x;\:B_{2..l}(x,q)=\frac{(q-\frac{1}{2})^{2}-\frac{1}{12}}{4}-\frac{(abs(x-q)-\frac{1}{2})^{4})-\frac{(abs(x-q)-\frac{1}{2})^{2}}{2}+\frac{7}{240}}{24}
$$
...
AIC és BIC alapján néztem a prediktorok bevonhatóságát természetes splineokkal (a $q_d$-k kvantilisek alapján lettek meghatározva).



Egy másodfokú ($d_1$=2) spline-nal érdemes bevonni a koplalás idejét, és egyszerű baseline-korrekcióval ($d_2$=1) az utolsó étkezéskor mért tömeget.  


```{r, fig.asp=.35, message = FALSE}
library(splines)
library(car)
...
A **\textcolor{red}{random hatásokra feltételes}** (tehát az étkezés utáni tömeg pontosan ismert!), bootstrap alapján becsült predikciós intervallum bizonytalansága igen kicsi.  
...
Az előzők a koplalás hatását specifikusan vizsgálják, hiszem - a baseline korrekció miatt - a tömegváltozást vizsgálják az étkezések után. Ennek kihagyása esetén ténylegesen a testtömeg változását nézhetjük az idő függvényében a következők szerint:
...
\sum_{k=1}^{d_1}\beta_{1k}\cdot B_k(Koplal\acute assal\:t\ddot olt\ddot ott\:id\hat o , q_d) \\
+ \sum_{k=1}^{d_1}\beta_{1k_i}\cdot B_k(Koplal\acute assal\:t\ddot olt\ddot ott\:id\hat o_i , q_d) \\
+\sum_{k=1}^{d_2}\beta_{2k}\cdot B_k(Eltelt\:napok\:sz\acute ama, q_d)+\epsilon_i
\end{split}
$$
$$
\alpha_i \in \textsl{N}(0,\sigma_{episode}^2),\: \epsilon_{i} \in \textsl{N}(0,\Sigma_i)
$$
...
BIC alapján $d_1$ = 2, $d_2$ = 3 ajánlott.  
